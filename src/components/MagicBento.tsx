...
const DEFAULT_GLOW_COLOR = '254, 101, 45'; // ✅ ORANGE #FE652D en RGB

// ✅ DONNÉES RESTAURANT (PHOTOS, NOMS, CATÉGORIES, PRIX)
const cardData: BentoCardProps[] = [
    {
        color: '#1c1917', // stone-900
        label: 'Entrée',
        title: 'Tartare de Saumon',
        description: 'Avocat, citron vert, aneth frais et huile de sésame.',
        price: '24€',
    ...
    },
    {
        ...
        gridClass: 'md:col-span-1 md:row-span-1' // Petite carte
    },
    {
        color: '#1c1917',
        label: 'Plat Signature',
        title: 'Homard Bleu',
        description: 'Beurre blanc, légumes croquants de saison.',
        price: '65€',
    ...
    },
    {
        color: '#1c1917',
        label: 'Plat',
        title: 'Agneau de Lait',
        description: 'Gratin dauphinois, jus au thym et romarin.',
        price: '42€',
        image: 'https://images.unsplash.com/photo-1544025162-d76694265947?auto=format&fit=crop&q=80&w=800',
        gridClass: 'md:col-span-2 md:row-span-1' // Carte horizontale
    },
    {
        color: '#1c1917',
        label: 'Dessert',
    ...
    },
    {
        ...
        description: 'Zestes frais, meringue italienne flambée.',
        price: '16€',
...
const MagicBento: React.FC<BentoProps> = ({
    ...
    enableTilt = false, glowColor = DEFAULT_GLOW_COLOR, clickEffect = true, enableMagnetism = false
}) => {
    ...
    return (
        <>
          ...
          .bento-section-wrapper {
            ...
            --border-color: rgba(254, 101, 45, 0.3); /* ✅ Bordure orange subtile */
          ...
          @media (min-width: 768px) {
            .card-responsive { grid-template-columns: repeat(4, 1fr); grid-auto-rows: minmax(180px, auto); }
          }
          .card--border-glow::after {
            content: ''; position: absolute; inset: 0; padding: 2px;
            background: radial-gradient(var(--glow-radius) circle at var(--glow-x) var(--glow-y),
                rgba(${glowColor}, calc(var(--glow-intensity) * 0.8)) 0%,
                rgba(${glowColor}, calc(var(--glow-intensity) * 0.3)) 40%, transparent 70%);
            border-radius: inherit;
            -webkit-mask: linear-gradient(#fff 0 0) content-box, linear-gradient(#fff 0 0);
            -webkit-mask-composite: xor; mask-composite: exclude;
            pointer-events: none; opacity: 0; transition: opacity 0.4s ease; z-index: 20;
          }
          .card--border-glow:hover::after { opacity: 1; }
          .text-clamp-1 { display: -webkit-box; -webkit-box-orient: vertical; -webkit-line-clamp: 1; overflow: hidden; text-overflow: ellipsis; }
          .text-clamp-2 { display: -webkit-box; -webkit-box-orient: vertical; -webkit-line-clamp: 2; overflow: hidden; text-overflow: ellipsis; }
        `}
            </style>

            {enableSpotlight && (
                <GlobalSpotlight gridRef={gridRef} disableAnimations={disableAnimations} enabled={enableSpotlight} spotlightRadius={spotlightRadius} glowColor={glowColor} />
            )}

            <div className="bento-section-wrapper" ref={gridRef}>
                <div className="card-responsive">
                    {cardData.map((card, index) => {
                        const baseClassName = `card flex flex-col relative w-full rounded-2xl border border-solid overflow-hidden transition-all duration-300 ease-out hover:-translate-y-1 hover:shadow-2xl ${card.gridClass} ${enableBorderGlow ? 'card--border-glow' : ''}`;
                        const cardStyle = { backgroundColor: card.color, borderColor: 'var(--border-color)', color: 'white' } as React.CSSProperties;
                        ...
                        return (
                            ...
                            >
                                ...
                                <div className="absolute inset-0 z-0">
                                    <img src={card.image} alt={card.title} className="w-full h-full object-cover opacity-50 group-hover:opacity-60 transition-opacity duration-500" />
                                    <div className="absolute inset-0 bg-gradient-to-t from-black/90 via-black/40 to-transparent" />
                                </div>

                                {/* ✅ CONTENU TEXTE PAR-DESSUS */}
                                <div className="relative z-10 flex flex-col justify-end h-full p-5 md:p-6 text-white">
                                    <div className="flex items-center justify-between mb-2">
                                        <span className="text-[#FE652D] text-xs font-bold tracking-[0.2em] uppercase">{card.label}</span>
                                        <span className="text-white/90 font-medium text-sm">{card.price}</span>
                                    </div>
                                    <h3 className={`font-serif text-xl md:text-2xl mb-2 leading-tight ${textAutoHide ? 'text-clamp-1' : ''}`}>
                                        {card.title}
                                    </h3>
                                    <p className={`text-stone-300 text-sm leading-relaxed ${textAutoHide ? 'text-clamp-2' : ''}`}>
                                        {card.description}
                                    </p>
                                </div>
                            </ParticleCard>
